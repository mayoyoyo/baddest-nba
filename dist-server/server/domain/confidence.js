...
export function calculateRankingConfidence(input) {
    if (input.totalImages <= 0) {
        return 0;
    }
    const counts = Array.from({ length: input.totalImages }, (_, index) => Math.max(0, input.comparisonCounts[index] ?? 0));
    ...
    const variance = counts.reduce((sum, count) => sum + (count - mean) ** 2, 0) /
        input.totalImages;
    const standardDeviation = Math.sqrt(variance);
    const balance = mean === 0 ? 0 : 1 - Math.min(1, standardDeviation / (mean + 1));
    return clamp01(coverage * 0.45 + volume * 0.35 + balance * 0.2);
}
export function calculateImageConfidence(input) {
