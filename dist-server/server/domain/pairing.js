...
export const RECENT_PAIR_CACHE_LIMIT = Math.max(EXACT_PAIR_COOLDOWN_MATCHUPS, RECENT_IMAGE_EXPOSURE_WINDOW);
function normalizePairKey(left, right) {
    return [left, right].sort().join(":");
}
function sortByAnchorNeed(left, right) {
    return (left.comparisons - right.comparisons ||
        left.confidence - right.confidence);
}
function sortExploratoryOpponents(anchor, left, right) {
    return (left.comparisons - right.comparisons ||
        left.confidence - right.confidence ||
        Math.abs(left.rating - anchor.rating) - Math.abs(right.rating - anchor.rating));
}
function sortRefinementOpponents(anchor, left, right) {
    return (Math.abs(left.rating - anchor.rating) - Math.abs(right.rating - anchor.rating) ||
        left.comparisons - right.comparisons ||
        left.confidence - right.confidence);
}
function compareDeprioritized(deprioritizedImageIds, leftImageId, rightImageId) {
    return (Number(deprioritizedImageIds.has(leftImageId)) -
        Number(deprioritizedImageIds.has(rightImageId)));
}
function buildBlockedImageIds(recentPairs) {
    ...
    occurrenceIndexes.forEach((indexes, imageId) => {
        const activeTrigger = indexes.some((pairIndex, index) => {
            ...
            return (nextOlderPairIndex - pairIndex <=
...
function buildRecentImageExposure(recentPairs) {
...
function compareRecentExposure(recentImageExposure, leftImageId, rightImageId) {
    return ((recentImageExposure.get(leftImageId) ?? 0) -
...
function createPairingRandom(input) {
    if (!input.recentPairs?.length) {
        return () => 0.25;
    }
    return createSeededRandom(hashString(JSON.stringify({
...
function withRandomFirst(items, random) {
    if (items.length < 2) {
        return items;
    }
...
function buildAnchorPool(anchors, deprioritizedImageIds) {
    const firstAnchor = anchors[0];
    if (!firstAnchor) {
        return [];
    }
    return anchors.filter((candidate) => compareDeprioritized(deprioritizedImageIds, firstAnchor.imageId, candidate.imageId) === 0 &&
        candidate.comparisons === firstAnchor.comparisons);
}
export function selectNextPair(input) {
    if (input.images.length < 2) {
        return null;
    }
    const cooledRecentPairs = (input.recentPairs ?? []).slice(0, EXACT_PAIR_COOLDOWN_MATCHUPS);
    const recentPairKeys = new Set(cooledRecentPairs.map(([left, right]) => normalizePairKey(left, right)));
    ...
    const candidateImages = input.images.filter((image) => !blockedImageIds.has(image.imageId)).length >= 2
        ? input.images.filter((image) => !blockedImageIds.has(image.imageId))
        : input.images;
    const remainingAnchors = [...candidateImages].sort((left, right) => compareDeprioritized(deprioritizedImageIds, left.imageId, right.imageId) ||
    ...
    while (remainingAnchors.length > 0) {
        const anchorPool = buildAnchorPool(remainingAnchors, deprioritizedImageIds);
        const orderedAnchors = withRandomFirst(anchorPool, random);
        for (const anchor of orderedAnchors) {
            const anchorIndex = remainingAnchors.findIndex((candidate) => candidate.imageId === anchor.imageId);
            if (anchorIndex >= 0) {
            ...
            const opponents = candidateImages
                ...
                .sort((left, right) => {
                return (compareDeprioritized(deprioritizedImageIds, left.imageId, right.imageId) ||
                    compareRecentExposure(recentImageExposure, left.imageId, right.imageId) ||
                    (exploratory
            ...
            const freshOpponent = opponents.find((candidate) => !recentPairKeys.has(normalizePairKey(anchor.imageId, candidate.imageId))) ?? opponents[0];
            if (freshOpponent) {
                return random() < 0.5
