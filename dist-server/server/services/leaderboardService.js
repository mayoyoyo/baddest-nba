...
function buildSharedStateRows(input) {
    ...
    const aggregateRows = aggregateSharedRanking(input.allUserStates.map((userState) => ({
...
export async function getSharedLeaderboard(db) {
    ...
    const sharedStateMap = new Map(sharedStateRows.map((row) => [row.imageId, row]));
    const winsByImage = new Map();
...
export async function getUserLeaderboard(db, username) {
...
export async function getPeople(db) {
    const [users, allUserStates] = await Promise.all([
        listUsers(db),
        listAllUserStates(db),
    ]);
    const stateByUserId = new Map(allUserStates.map((state) => [state.user_id, state]));
    return {
        users: users
            ...
            .sort((left, right) => right.summary.totalVotesCast - left.summary.totalVotesCast ||
